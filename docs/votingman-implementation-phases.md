...
| **7** | **티어·MMR** | 시즌·배치고사·MMR·티어 (시장별) | 정산 이력·승률 위에 구축. 규칙 조정 가능. |
| **8** | **리더보드 + 홈** | 시장별 TOP 5, 현재 포지션 위젯 | 7단계 위 UI. 노출 방식 변경 가능. |
...
- **정산**: 순수 파리뮤추얼. 당첨자 i 수령 = 패자_풀 × (본인 베팅 코인 / 당첨측 총 베팅 코인).
...
- **정산 로직 (순수 파리뮤추얼)**
  ...
  - **분배**: 패자 풀을 당첨자들에게 **베팅 코인 비율**로만 나눠 줌.  
    - 수식: 당첨자 i의 수령 = 패자_풀_전체 × (bet_i / 당첨측_총_베팅).  
...
- **누적 승률**과 **티어**는 해당 시즌·해당 시장에서 **최소 배치 5판**을 치른 뒤에만 확정·노출.
- **5판 미만**: 티어를 “**티어 없음(배치고사 5판 필요)**”로 표시. 배치고사를 보지 않은 유저도 이 문구를 보면 “아, 배치고사 5판을 해야 티어가 생기는구나”를 인지할 수 있도록 함. 누적 승률은 5판 미만일 때 MMR/티어 계산에 사용하지 않음.
...
- **배치**: 시즌마다 최초 5판 결과로 해당 시즌 배치 완료. 이후 시즌 누적 승률·티어 계산.
...
- 시즌이 연 3회로 구분되고, 시즌마다 배치고사(5판) 후 티어·MMR 확정.
- MMR = 보유 코인 × 시즌 누적 승률로 계산·저장되며, 배치 결과·하한 보정 반영. 리더보드/티어 UI에 반영됨.

---

## 7단계: 리더보드 + 홈 고수 포지션

**목표**: **시장(탭)별로 리더보드를 나누어**, 각 영역에서 TOP 5와 **현재 포지션**(해당 시장 오늘 롱/숏)을 보여 준다. 코인 고수·미국 주식 고수·한국 주식 고수가 각각 구분되어 노출된다.

**시장별 리더보드 구조**

| 탭(시장) | 리더보드 | 현재 포지션 의미 |
|----------|----------|------------------|
| **비트코인** | 비트코인 리더보드 (해당 영역 TOP 5) | 오늘 비트코인 폴 투표(롱/숏) |
| **미국 주식** | 미국 주식 리더보드 (해당 영역 TOP 5) | 오늘 나스닥·S&P500 폴 투표(롱/숏) |
| **한국 주식** | 한국 주식 리더보드 (해당 영역 TOP 5) | 오늘 코스피·코스닥 폴 투표(롱/숏) |

- **코인 탭** → 코인(비트코인) 리더보드만 노출.  
- **미국 주식 탭** → 미국 주식 리더보드만 노출.  
- **한국 주식 탭** → 한국 주식 리더보드만 노출.  
- 각 탭에서 “현재 포지션”은 **그 시장의 오늘 투표(롱/숏)**를 의미함.

**리더보드 노출 항목 (시장별 TOP 5 고정)**

| 노출 항목 | 설명 |
|-----------|------|
| 닉네임 | 유저 닉네임 |
| MMR | 랭킹·티어 점수 (보유 코인 × 누적 승률). 시장별 리더보드면 해당 시장 MMR. |
| 가진 자산(보팅코인) | 현재 보팅코인 잔액 |
| 누적 승률 | 해당 시즌 누적 승률 (시장별이면 해당 시장 기준) |
| 현재 포지션 | **해당 시장** 오늘 투표(롱/숏) — 실시간 |
| 배팅한 코인 수 | **해당 시장**에서 마지막(또는 오늘) 배팅한 코인 수 |

**구현 내용**

- **리더보드**
  - **시장(탭)별**로 MMR 기준 **TOP 5** 조회. (비트코인 / 미국 주식 / 한국 주식 각각 해당 시장 MMR·랭킹 사용.)
  - 각 탭/블록: **닉네임, MMR, 가진 자산(보팅코인), 누적 승률, 현재 포지션, 배팅한 코인 수** 노출.
- **현재 포지션**
  - 해당 탭의 시장(비트코인 / 미국 주식 / 한국 주식)에 해당하는 **오늘 폴**의 투표(롱/숏)를 TOP 5 유저별로 조회해 “현재 포지션” 컬럼에 표시. (미국 주식은 나스닥·S&P 두 폴, 한국 주식은 코스피·코스닥 두 폴이 있을 수 있으므로, “해당 시장 대표 1폴” 또는 “두 폴 모두 표시” 등은 UI 정책에서 결정.)
- **홈 배치**
  - 홈 또는 인간 지표 영역에서 탭(비트코인 | 미국 주식 | 한국 주식)에 따라 해당 시장 리더보드 + 현재 포지션 위젯 노출.

**완료 기준**

- 비트코인·미국 주식·한국 주식 탭 각각에서 해당 시장 TOP 5 리더보드가 노출되고, 닉네임·MMR·가진 자산(보팅코인)·누적 승률·현재 포지션·**배팅한 코인 수**가 표시됨.

---

### 시장별 초기 지급·MMR·랭킹·티어 분리 시 (복잡도와 선택지)
...
**확정: 잔액 1개 + 시장별 MMR·랭킹·티어**

- **보팅코인 잔액은 1개**로 유지. 모든 투자자의 **시드머니도 1개**이며, 그 하나의 자산으로 **어떤 포트폴리오(어떤 시장에 얼마나 걸지)**로 투자할지는 각자의 자유다. 따라서 잔액·초기 지급은 시장별로 나누지 않음.
- **MMR, 랭킹, 티어만 시장별**로 둠. 시장별 승률만 따로 저장 → 시장별 MMR = (전역 잔액) × (해당 시장 시즌 누적 승률). 시장별 배치 5판·시즌 전환·하한/상한은 “해당 시장 폴”만으로 계산.
- **티어**는 시장별로 두면 (비트코인 골드, 미국주식 플레티넘 …) 3개. DB에는 시장별 MMR·티어 컬럼 또는 시장별 프로필 행을 두면 됨.
...
| **시장별 배치 5판** | **시장별 배치 5판 존재.** 비트코인 5판, 미국주식 5판, 한국주식 5판 각각. **매 시즌마다 초기화.** |
...
|------|----------------------|-----------------|------|
| **비트코인** | 다음날 00:00 | **20:30** | 장 마감 없음 → 예외적으로 20:30 |
...
- 2025-02: 최종 확정 — 어뷰징 방지 현재 단계 미적용(훗날 업데이트). 회원가입/로그인 **구글 + 카카오만**. 배팅 수정·취소 가능. 시장별 투표 마감: 비트코인 **20:30**, 한국 주식 **13:00**, 미국 주식 **03:30** KST. 미국 일봉 종가 일괄 6시 KST(서머타임 추후 업데이트). 비트코인 마감 20:30 코드 반영.
