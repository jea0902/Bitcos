 ...
 *
 * 설계 의도:
 * - 투표 허용 시간: KST 00:00 ~ 22:30 (마감 22:30)
...
/**
 * 오늘 KST 00:00 ~ 22:30 이내이면 투표 허용.
 * 22:30 KST 정각 및 이후에는 false (투표 불가).
 * 클라이언트·서버 공용 (API에서도 동일 조건 적용 권장).
 */
export function isVotingOpenKST(): boolean {
  const mins = getKSTMinutesSinceMidnight();
  const closeAt = 22 * 60 + 30; // 22:30 KST
  return mins < closeAt;
}

/** 마감 시각 라벨 (표기용) */
export const VOTING_CLOSE_LABEL = "마감 22:30 (KST)";
